...
class MailServiceTest {
    ...
    public void testSendSimpleMail() {
        // 请将收件人邮箱替换为实际测试邮箱
        String to = "244859845@qq.com";
        String subject = "Spring Boot 邮件测试";
        String content = "这是一封通过 Spring Boot Mail 发送的简单邮件！";
    ...
    // 新增：富文本激活邮件测试
    @Test
    public void testSendActivateHtmlMail() {
        // 替换为你的收件人邮箱
        String to = "244859845@qq.com";
        ...
        System.out.println("富文本激活邮件发送成功！");
    }

    @Test
    public void testSendMailWithTwoAttachments() {
        String to = "244859845@qq.com"; // 替换为你的收件邮箱
        String subject = "带双附件的测试邮件";
        String content = "<p>您好！<br>这是一封包含 <strong>两个附件</strong> 的测试邮件。</p>";

        List<File> attachments = new ArrayList<>();
        try {
            // 附件 1: 从 resources 目录读取文本文件
            ClassPathResource resource1 = new ClassPathResource("test_attachment.txt");
            attachments.add(resource1.getFile());

            // 附件 2: 从 resources 目录读取图片文件
            ClassPathResource resource2 = new ClassPathResource("test_image.png");
            attachments.add(resource2.getFile());

            // 执行发送
            mailService.sendMailWithAttachment(to, subject, content, attachments);
            System.out.println("✅ 带双附件的邮件发送成功！");
        } catch (Exception e) {
            e.printStackTrace();
